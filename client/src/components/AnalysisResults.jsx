...
import { CheckCircle, AlertTriangle, XCircle, Download, RefreshCw, Clock, Pill } from 'lucide-react';

const AnalysisResults = ({ results, onNewAnalysis }) => {
  if (!results) return null;

  // Parse ai_analysis if it's a string
  const parsedResults = typeof results.ai_analysis === 'string' ? JSON.parse(results.ai_analysis) : results.ai_analysis;
  ...
  const downloadReport = () => {
    const reportData = {
      ...
      medicines: parsedResults.medicines,
      interactions: parsedResults.interactions,
      recommendations: parsedResults.recommendations,
      blobName: results.blob_name,
    };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            {getRiskIcon(parsedResults.riskLevel)}
            Overall Risk Assessment
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-3 mb-3">
            <Badge className={getRiskColor(parsedResults.riskLevel)}>
              {parsedResults.riskLevel?.toUpperCase() || 'UNKNOWN'} RISK
            </Badge>
            <span className="text-sm text-muted-foreground">
              Analysis completed at {new Date(results.analyzed_at).toLocaleString()}
            </span>
          </div>
          <p className="text-muted-foreground">
            {parsedResults.summary || 'Analysis completed successfully.'}
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Extracted Medicines ({parsedResults.medicines?.length || 0})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            {parsedResults.medicines?.map((medicine, index) => (
      ...
      {parsedResults.interactions && parsedResults.interactions.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {parsedResults.interactions.map((interaction, index) => (
                <Alert key={index}>
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    <div className="space-y-1">
                      <div className="flex justify-between items-start">
                        <strong>{interaction.drug1 && interaction.drug2 ? `${interaction.drug1} + ${interaction.drug2}` : 'Drug Interaction'}</strong>
